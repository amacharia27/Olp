// apps/frontend/src/features/official/county/pages/CountyTransferApprovalPage.tsx
...
const mockTransferApplications: TransferApplication[] = [
  {
    ...
    approvalChain: {
      ...
      subcounty: {
        status: 'approved',
        comment: 'Approved based on headteacher recommendation',
        date: '2025-05-25',
      },
      county: {
        status: 'pending',
  ...
  },
  {
    ...
    approvalChain: {
      ...
      subcounty: {
        status: 'approved',
        comment: 'Transfer is justified based on family needs',
        date: '2025-06-18',
      },
      county: {
        status: 'pending',
  ...
  },
  {
    ...
    approvalChain: {
      ...
      subcounty: {
        status: 'approved',
        comment: 'Recommended for advancement',
        date: '2025-06-22',
      },
      county: {
        status: 'pending',
  ...
  },
  {
    ...
    approvalChain: {
      subcounty: {
        status: 'approved',
        comment: 'Transfer recommended due to health considerations',
        date: '2025-06-10',
      },
      county: {
        status: 'pending',
...
const CountyTransferApprovalPage = () => {
      ...
      const updatedApplications = applications.map(app => {
        if (app.id === currentApplication?.id) {
          return {
            ...
            status: 'approved' as const, // Final approval from county
          };
        }
        return app;
      });
      ...
      const updatedApplications = applications.map(app => {
        if (app.id === currentApplication?.id) {
          return {
            ...
            status: 'rejected' as const, // Rejected by county
          };
        }
        return app;
      });
      ...
      const updatedApplications = applications.map(app => {
        if (app.id === currentApplication?.id) {
          return {
            ...
            status: 'objected' as const, // Objected by county
          };
        }
        return app;
      });
  ...
  const columns: ColumnsType<TransferApplication> = [
    ...
    },
    {
      ...
      key: 'county',
      render: (_, record) => {
        const status = record.approvalChain.county?.status || 'pending';
        let color = 'gold';
        if (status === 'approved') color = 'green';
        else if (status === 'rejected') color = 'red';
        else if (status === 'objected') color = 'orange';
    ...
    },
    {
      ...
      key: 'actions',
      render: (_, record) => {
        // Only show review button if subcounty has approved and county status is pending
        const isReviewable = 
          record.approvalChain.subcounty?.status === 'approved' && 
          record.approvalChain.county?.status === 'pending';
  ...
  const filteredApplications = applications.filter(app => {
    if (activeTab === 'pending') {
      return app.approvalChain.county?.status === 'pending';
    } else if (activeTab === 'approved') {
      return app.approvalChain.county?.status === 'approved';
    } else if (activeTab === 'rejected') {
      return app.approvalChain.county?.status === 'rejected';
    } else if (activeTab === 'objected') {
      return app.approvalChain.county?.status === 'objected';
    }
    return true;
  });

  const pendingCount = applications.filter(app => 
    app.approvalChain.subcounty?.status === 'approved' && 
    app.approvalChain.county?.status === 'pending'
  ).length;

  const approvedCount = applications.filter(app => app.approvalChain.county?.status === 'approved').length;
  const rejectedCount = applications.filter(app => app.approvalChain.county?.status === 'rejected').length;
  const objectedCount = applications.filter(app => app.approvalChain.county?.status === 'objected').length;

  return (
    <div>
      <Title level={2}>County Transfer Applications Review</Title>
      <Text>As a county official, you need to review transfer applications that have been approved by subcounty officials.</Text>
      ...
      >
        {currentApplication && (
          <div>
            ...
            {currentApplication.approvalChain.subcounty && (
              <div>
                <p><strong>Subcounty Decision:</strong> {currentApplication.approvalChain.subcounty.status.toUpperCase()}</p>
                <p><strong>Subcounty Comments:</strong> {currentApplication.approvalChain.subcounty.comment}</p>
              </div>
            )}
...
export default CountyTransferApprovalPage;
